...
from peewee import (Model, CharField, IntegerField,
                    ...
                    IntegrityError, SqliteDatabase)

DATABASE_PATH = join(dirname(dirname(abspath(__file__))), 'data')
DATABASE = SqliteDatabase(join(DATABASE_PATH, 'reports.db'))
    ...
    def delete_manager_report(self, report_instance=None, benchmark_id=None):
        with self.database:
            if report_instance and isinstance(report_instance, ManagerReport):
                ...
                deleted = ManagerReport.select().where(
                    ManagerReport.benchmark_id == benchmark_id).execute()
                if deleted:
                    return True

        return False

    def update_manager_report(self,
                              current_case_index,
                              current_case_id,
        ...
        with self.database:
            if report_instance and isinstance(report_instance, ManagerReport):
                ...
                ManagerReport.update({
                    ...
                    ManagerReport.benchmark_id == benchmark_id).execute()
                report_instance = self.select_manager_report(benchmark_id=benchmark_id)
            else:
                report_instance = None
    ...
    def delete_ai_report(self,
        ...
        with self.database:
            if report_instance and isinstance(report_instance, AIReport):
                ...
                deleted = AIReport.select().where(
                    AIReport.manager_report == manager_report,
                    AIReport.ai_name == ai_name,
                    AIReport.case_id == case_id).execute()
                if deleted:
                    return True

        return False

    def update_ai_report(self,
                         manager_report,
                         ai_name,
                         case_id,
                         case_status,
