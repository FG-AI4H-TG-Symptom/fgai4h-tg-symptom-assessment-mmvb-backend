...
from evaluator.benchmark.definitions import (
    CaseStatuses, MAX_NUM_ATTEMPTS, MAX_WAIT_BETWEEN_RETRIES,
    SOLVE_CASE_SOFT_TIMEOUT, SOLVE_CASE_HARD_TIMEOUT)
...
class BenchmarkRunner(Process):
    ...
    def _perform_request(self,
        ...
        response = requests.request(
            method, url, params=parameters, data=json.dumps(data), headers=headers,
            timeout=timeout
        )
        return response
    ...
    def _log_error(self, message=None, status_code=None, data=None, exception=None):
        if not message:
            message = (f'Max attempts of {MAX_NUM_ATTEMPTS} retries failed '
                       f'for {self.ai_name}.')
    ...
    def _log_info(self, message=None, status_code=None, data=None, exception=None):
        if not message:
            message = (f'Health check attempt #{self.healthcheck_attempts} failed '
                       f'for {self.ai_name}.')
    ...
    @retry(stop_max_attempt_number=MAX_NUM_ATTEMPTS,
